...
V1.29 : 2025/03/05
		.Android-15 対応
        .折り畳みデバイス対応
        .プロフールアイコンを設定しないと和了処理が進まない不具合対処
...
V1.28 : 2023/02/25
        ...
        .役の確定の整理
          -先付けの時第二副露以降は役との関連を問わない
          -先付けの時第一副露が役と関係ないとき確定待ちでも後付けとみなす
          -役牌のシャンポン待ちはカンチャン、ペンチャンと同じ確定待ちとする
        .不具合
          -無関係な鳴きが挟まると三同刻を判断できない
          -数牌が頭だけで後はすべて字牌のとき役の判断ができない
          -混老の翻数の誤り。４翻ではなく２翻が正しい
          -役牌の先付けエラーがあるとき小三元を判定しない
          -役牌が最初の鳴きであっても他のなきが入ると先付けエラーなる
          -役牌と他牌のシャンポン待ちのとき片上がりエラー翻数設定されない
          -頭が白のとき小三元を評価できてなかった
          -三暗刻、三同刻で、刻子と順子が重なるとき役の判定を誤る

V1.27 : 2023/01/31
		.捨て牌のサイズを少し大きくした
        .不具合
		 -Android12以降、Bluetoothデバイス探索が失敗することがある
         -アンインストール+インストールで"問題が発生しました"が出ることがある


V1.27 : 2023/01/31
		.配牌の乱数発生を xorshift に変更
		.細長画面の判定をアスペクト比 1.8から16:9=1.77(スマホの主流)に変更。タブレットは16:10=1.6, 4:3=1.33が多い
		.小画面サイズの判定を800-->720ピクセルに変更
		.得点ボタンは和了要求画面を閉じたときはオレンジに戻す
		.局の終了画面を閉じたときメニューボタンをオレンジにする
		.局の終了画面をにプロファイルアイコンを表示する
		.接続ダイアログ(BT/Wifi)に接続前の状態の時接続歴からユーザー名も表示する
		.デバイスリストの無駄な再描画回数を減らす
		.捨て牌のサイズを少し大きくした
        .不具合
		 -requestPermission 同時には出せないのでENQ して制御する
         -BLUETOOTH_ADVERTIZEは間違いでBLUETOOTH_ADVERTISEが正しい(Android12以降Scanで必要)
         -Dounloadフォルダーはドキュメントフォルダーとして使用できないので再設定が必要
         -UMediastore query で列数=0のときAbendする
         -Android21以下の時取り消しボタンが反転しない
         -アンインストール+インストールで"問題が発生しました"が出ることがある

V1.26 : 2022/10/24
		.Api33(Android13:T)対応

V1.26 : 2022/10/24
		.接続があるときに個人設定の変更は防止する(プロファイルアイコン)
		.Java9での非推奨に対応
		.Api33(Android13:T)対応
		.Api31(Android12:S)対応もれ
		.ゲーム中ダイアログのルールボタンがRuleSettingSummary呼び出しになってなかった
		.縦横比によるスマホデバイスの判定を緩める(２倍-->1.8倍)
		.最初のインストール後ルールファイルの初期を暗黙でする

V1.25 : 2022/10/14
		.Bluetooth接続の権限に関連してAndroid-12対応
		.履歴の印刷機能を追加
        .不具合(４人プレーゲームの履歴の集約、接続が切れたときの対応)

V1.25 : 2022/10/14
		.Bluetooth接続の権限に関連してAndroid12対応
		.履歴の印刷機能を追加
        .不具合
		 -４人プレーゲームを選択した時、履歴の集約が全ゲームの集約になってしまう
		 -機種によってはプロフィールICONが牌山に重なることがある
		 -接続が切れたときメッセージがループする

V1.24 : 2022/10/08
		.代表的なルールの設定項目を1ページに集約
		.個人設定によるプロフィールアイコン表示をサポ－ト
		.接続するときアプリのバージョンが一致するかチェックする
        .(不具合) 初めてのインストールの場合ルールの同期に失敗する。

V1.24 : 2022/10/08
		.代表的なルールの設定項目を1ページに集約
		.一部メッセージの韓国語対応
		.横向きの時の縦書きメッセージの文字間スペースを調整
		.個人設定によるプロフィールアイコン表示をサポ－ト
		.接続するときアプリのバージョンが一致するかチェックする
        .不具合
		 -初めてのインストールの場合ルールの同期に失敗する。

V1.23 : 2022/08/25
		.流し満貫の判定、八連荘、形式聴牌をインプリメント
		.オープン立直オプションの変更。
		 また通常立直後のオープン立直への振り込みは役満とはしない。ロボットも役満払い。
		.接続時にアプリのバージョン合わせをする。
        .不具合
		 -加槓に対する槍槓のフリテン。一盃口/三暗刻の先付け違反。二翻縛りの時、２翻めの役牌の先付け違反。
		 -履歴からの中断ゲームの再開がらみ。
...
V1.23 : 2022/08/25
		...
		.形式聴牌をインプリメント
         無しは、ロン上がりできるとき聴牌とする。
         有リはフリテン、片上がり、先付けエラーはOK。縛りはオプションとする。
         ノーテン立直がありうるので立直だけで聴牌とはみなさない。
		.オープン立直の振り込みをインプリメント、また警告オプションも追加。
		 加槓の時もオープン立直の槍槓にならないかチェックする。
		 また通常立直後のオープン立直への振り込みは役満とはしない。
		.ロボット入りゲームのオープン立直オプションは有無のみとし、払いは役満とする。
		."フリテンをチェックする"がOFFの設定の時フリテンエラーのメッセージは出さないがボタン通知はしない。
		 デフォルトはONに変更する。
		.ルールファイルの変更に併せてAPPのバージョンを合わせをする。
		.ロボットの強化
         -七対子のドラ待ちは立直しない。
		 -オープン立直がかかっているときは槓はしない。
		 -オープン立直に対しフリテンをチェックして捨て牌を選ぶ。
		 -七対子は１シャンテンのときできるだけ字牌を捨てない。
		 -七対子の立直の時、役牌待ちは避ける。
        .不具合
		 -加槓に対する槍槓の同順フリテンチェックをしていなかった。
		 -一盃口/三暗刻の先付け違反のチェックが出来ていなかった。
		 -二翻縛りの時、２翻めの役牌の先付け違反チェックに誤りがあった。
		 -ロボット入りのとき"和了は取り消し有り"にすると進行が止まることがある。
		 -履歴からの中断ゲームの再開がらみ。
          .中断ゲームで使用したルールファイルが、そうでないゲームに継続される。
          .中断ゲームがゲームオーバーしたときで使用したルールファイルが削除され、
           他の同じルールファイルを使用する中断ゲームが再開できない。
          .中断ゲームがゲームオーバーしても、中断ゲームが履歴リストに残されている。

V1.22 : 2022/07/16
		.ヘルプといくつかのメッセージダイアログでハングル表示を追加

V1.21 : 2022/04/23
        .Andoid12(API31)対応
        .ドラ牌、和了、鳴き、立直をアニメーションで表示する
V1.21 : 2022/04/23
        .Andoid12(API31)対応
        .ドラ牌、和了、鳴き、立直をアニメーションで表示する
        .ヘルプダイアログにテキストズーム機能追加
        .ミスタッチを避けるためとに和ボタンを左端に移動
        .メニューの対局終了の状況エラーの時どのメニューを使用すべきかを指示する
        .ルール同期が必要になるのを避けるため一人遊びのオプションを操作設定から個人設定に移動。
        .不具合
         -最初のルールダイアログ表示に "ありありなど" と日本語が表示されている
         -EmulatorではBluetoothがEnableにできないが、Enable要求がループしてダイアログを閉じれない
         -一人遊びの時鳴いた後、自摸を押すと"あなたの番ではありません"が出る。"打牌せよ"とすべき。
         -対戦通知モードで自動自摸したときボタン背景色が残る

V1.20 : 2022/03/24
        .フリテン立直オプションをインプリメントする。
        .四開槓、四風連打、四人立直の途中流局インプリメント
        .ルールの送信を受けたとき相違点をカラー表示する
        .ロボット強化、不具合修正
V1.20 : 2022/03/24
        .とに和のとき１飜縛りはチェックしない
        .後付け有りの時は片上がりは全てＯＫとする。後付けでないときの自摸はオプションとする
        .フリテン立直オプションをインプリメントする。
        .片上がりエラーと１飜縛りエラーを区別する
        .和了確認、局の終了ダイアログにプレーヤー名を表示する
        .四開槓、四風連打、四人立直は自動で途中流局ダイアログをオープンする。したがって打牌＋流局ボタンは廃止
        .ルールの送信を受けたとき相違点をカラー表示する
        .ルールの送信を受けたとき操作、役の子ダイアログが表示されていたらクローズする
        .ロボット強化
         -ロボットも嶺上開花する
        .不具合
         -三色、一通の中付け判定
         -白發中の訳数がクリアーされずに多くカウントされていた
         -小三元も先付ルールに従う
         -三暗刻も片上がりチェックが必要
         -途中流局しダイアログをキャンセルするとハングすることがある
         -国士無双がの暗槓ロンが暗槓が自摸直後でないと失敗する
         -ロボットの他プレーヤーの一色警戒の判断ミス
         -ロボット一通確定のチーの判定の誤りによりDumpすることがある
V1.19 : 2022/02/01
        .フリテン立直エラーを表示する
        .ロボット強化
        .不具合の修正
V1.19 : 2022/02/01
        .デバッグモーで無くても例外発生は Logcat に書く
        .履歴ダイアログのゲームリストで一人遊びは色を変える
        .AndroidStudio DebugRun のパフォーマンスのためテストオプションにNoDumpオプション追加
        .ネームプレートの名前を中央に表示
        .Top Panel の ヘルプから YouTube 連携
        .立直メッセージは立直コールしたときには出さず、打牌したときに出す
        .立直メッセージは立直コールしたときには出さず、打牌したときに出す
        .フリテン立直の警告は拒否オプションの時だけでなくフリテン立直ありの時にも出す
        ."フリテン立直オプション＝Noのとき自摸も上がれない"を組み込む
        .立直後の和了牌の見逃しをチェックする（上がれない）
        .フリテン立直オプション＝Noのときのフリテン立直、立直後の和了牌の見逃しは、和了、途中流局、流局のダイアログに表示する
        .暗槓の通知はしない、加槓は自摸の時のみ通知する
        .接続ボタン、ゲームタイプテキストボックスの背景色をサーバーとクライアントで変える
        .ロボット強化
         -立直者が１人の時、ドラがなければ１飜確定でも鳴かない
         -一度鳴いているからと言って鳴く理由にはしない。
         -鳴くか否かの判定の後に他のプレーヤーをチェックしてギブアップ判定していたが、鳴く前にもギブアップ判定する。
         -捨て牌の検討で和了牌からフリテンは除く
         -チャンタの志向判定で数牌３は１と２がないとき、７は８と９がないときタンヤオ牌と考える
         -オーラスでは高得点を目指す
         -１色志向ではチーする前に、他色の牌数を考慮する
         -７対子志向でもダブ東はシャンテン >= ２ ならポンする
         -７対子志向のとき誰か立直していればシャンテン<=1でも警戒する。（捨て牌が選びやすいので）
         -立直を警戒するとき字牌も牌数、役牌かどうかで差をつける
         -７対子志向の捨て牌の選択で赤ドラ近辺、字牌は役牌以外を残すようにする
         -７対子志向の捨て牌の選択で現物は捨てやすく、筋牌、字牌は捨てにくくする
         -７対子単騎待ちの時、タンヤオ志向のならドラ待ちでない限りタンヤオ牌を自模るのまで立直を待つ
         -７対子単騎待ちの時、チャンタ志向のならドラにかかわらずチャンタ牌を自模るのまで立直を待つ
         -７対子はチャンタペア>４のときチャンタ志向とする（＝４のときはチャンタ孤立牌＞＝２のときチャンタとする）
         -"役牌は先付でなければ鳴いて１飜"に従う
         -明槓は先付１飜を失う可能性があれば鳴かない
         -７対子単騎待ちの時、１色志向ならドラに関係なく１色になる牌を自模るのまで立直を待つ
        .不具合
         -ＢＧＭの "未設定" が英語環境でも日本語で表示される
         -途中流局要求機能は無くしたがゲーム中メニューの説明に残っていた。ヘルプを修正した。
         -途中流局要求は配牌前は拒否すべきだが、２回目の2局以降はできてなかった。
         -鳴くか否かの判定でドラ数を利用するがその時カンドラが考慮されてなかった。上がりの時はカウントされていた。
         -Try-Catch は Alet の OnClick でも必要（Crash 予防)。
         -途中流局確認が複数表示されたとき。Dismiss でクラッシュする(OnDismiis でポインターをnullにしていた）
         -途中流局確認をキャンセルしたときゲーム中でないのにAlert が出る
         -履歴から送信するとき、ヒストリールールファイルが SDCARD システムと Scoped File システムとではDumpする
         -位置決め簡略モードの時ランプをEnableにしない。２重タッチとなる恐れがある。
         -立直しない判断に至ったとき牌の価値が２倍に増幅される
         -通知モードでない対戦の時、クライアントの和了ボタンで１飜縛りエラーになることがある
         -槍槓が上がれない
         -クライアントから"Game"ボタンを押すことができる
         -"Game" 開始の時クライアントが向きの選択ダイアログをキャンセルしたので、サーバーがトップ画面に戻るとクライアントがNPEになる
         -クライアントの精算後のネームプレートのポイントの背景色がサーバーと異なる
V1.18 : 2021/11/26
        .チーの副露がおかしい不具合の修正
